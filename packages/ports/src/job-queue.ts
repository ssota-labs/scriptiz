...
export interface EnqueueOptions {
  /** Skip duplicate check against running/ (for tests or admin). Default false. */
...
export interface JobQueuePort {
   ...
   * @throws {DuplicateRunningJobError} if a conflicting job is in running/
   */
  enqueue(
    job: ExtractionJob,
  ...
  /** True if any job in running/ targets the same resource or same sourceUrl+kind. */
...
export class DuplicateRunningJobError extends Error {
  ...
  constructor() {
    super("A job for this resource or URL is already running");
  }
}
